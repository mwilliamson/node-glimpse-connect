...
DataStore.prototype.addMiddleware = function(requestId, middleware, stack) {
    var data = this._get(requestId);
    data.middleware = data.middleware || [];
    
    var stackElement = _.find(stack, function(element) {
        return element.fun === middleware;
    }) || {};
 
    data.middleware.push({
        name: middleware.name,
        source: {
            path: stackElement.path,
            line: stackElement.line,
            col: stackElement.col
        }
    });
}
...
function generateMiddlewareTab(data) {
    var tabData = [["Name", "Path", "Line", "Col"]];
